...
import { ArrowLeft, Eye, EyeOff, LoaderCircle, Lock, Mail, Save, User, UserCheck } from 'lucide-vue-next';
...
const isEdit = computed(() => !!props.reviewer);

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Reviewer', href: '/admin/reviewers' },
...
function submit() {
    if (isEdit.value && props.reviewer) {
...
<template>
    <Head :title="isEdit ? 'Ubah Reviewer' : 'Tambah Reviewer'" />
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto flex w-full max-w-lg flex-col gap-6 p-4">
            ...
            >
                <ArrowLeft class="size-4" /> Kembali ke Daftar Reviewer
            </Link>

            <PageHeader
                :icon="UserCheck"
                icon-class="bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-400"
                :title="isEdit ? 'Ubah Akun Reviewer' : 'Tambah Akun Reviewer'"
                :description="isEdit ? `Perbarui data akun ${reviewer?.name}.` : 'Buat akun baru untuk menilai artikel yang masuk ke SNOS 2026.'"
            />
