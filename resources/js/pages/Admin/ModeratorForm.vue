...
import { ArrowLeft, Eye, EyeOff, LoaderCircle, Lock, Mail, Mic, Save, User } from 'lucide-vue-next';
...
const isEdit = computed(() => !!props.moderator);

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Moderator', href: '/admin/moderators' },
    { title: isEdit.value ? 'Ubah Akun' : 'Tambah Akun', href: '#' },
];

const form = useForm({
    name: props.moderator?.name ?? '',
...
function submit() {
    if (isEdit.value && props.moderator) {
        form.put(route('admin.moderators.update', props.moderator.id));
    } else {
...
<template>
    <Head :title="isEdit ? 'Ubah Moderator' : 'Tambah Moderator'" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto flex w-full max-w-lg flex-col gap-6 p-4">
            <Link
                :href="route('admin.moderators.index')"
                class="inline-flex w-fit items-center gap-1.5 text-sm text-muted-foreground transition-colors hover:text-foreground"
            >
                <ArrowLeft class="size-4" /> Kembali ke Daftar Moderator
            </Link>

            <PageHeader
                :icon="Mic"
                icon-class="bg-violet-100 text-violet-700 dark:bg-violet-950/60 dark:text-violet-400"
                :title="isEdit ? 'Ubah Akun Moderator' : 'Tambah Akun Moderator'"
                :description="isEdit ? `Perbarui data akun ${moderator?.name}.` : 'Buat akun baru untuk memandu sesi presentasi SNOS 2026.'"
            />
                ...
                <CardContent class="pt-6">
                    <form class="flex flex-col gap-5" @submit.prevent="submit">
                        ...
                        <div class="mt-2 flex items-center gap-3">
                            ...
                            <Link :href="route('admin.moderators.index')">
